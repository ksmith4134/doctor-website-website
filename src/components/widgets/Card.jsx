...
export default function Card(props) {
    ...
    return (
        <div className={`p-px rounded-4xl ${BACKGROUND.GRADIENT_DIRECTION[backgroundGradient]} from-white/5 from-0% to-gray-400/5 to-50% overflow-hidden`}>
            <div className="relative w-full h-full rounded-4xl bg-gray-950">
                <div className="w-full h-full rounded-4xl overflow-hidden bg-noise bg-repeat">
                    <div className={`w-full h-full px-12 py-14 rounded-4xl ${BACKGROUND.GRADIENT_DIRECTION[backgroundGradient]} from-[#565A67]/10 to-transparent shadow-inner shadow-gray-600/5`}>
                        <div className="w-fit px-4 py-2 rounded-full bg-gray-400/70 text-xs font-medium text-gray-950">{kicker}</div>
                        <div dangerouslySetInnerHTML={{__html: title}} className="mt-10 font-hind text-4xl font-medium text-gray-300 leading-snug"></div>
