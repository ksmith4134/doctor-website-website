...
const credentials = [
    { id: 0, label: "Boards", list: [{ id: 0, label: "American Board of Orthopedic Surgery" }] },
    { id: 1, label: "Licensed", list: [{ id: 0, label: "New York" }] },
    { id: 2, label: "Professional Organizations", list: [{ id: 0, label: "American Academy of Orthopedic Surgeons" },] },
    { id: 3, label: "Awards & Recognitions", list: [{ id: 0, label: "Alpha Omega Alpha Honor Medical Society" },] },
];

const education = [
    { id: 0, label: "BS, Biology", link: "Syracuse University" },
    { id: 1, label: "MD", link: "Syracuse University" },
    { id: 2, label: "Residency, Orthopedic Surgery", link: "SUNY Upstate Medical University" },
    { id: 3, label: "Fellowship, Sports Medicine", link: "Orthopaedic Research Institute" },
                ...
                <div className="p-12 bg-portfolio-primary/10">
                    ...
                    {credentials.map((item) => (
                        <div key={item.id} className="mt-8">
                            ...
                            {item.list.map((item) => (
                                <h5 key={item.id} className="mt-2 text-xs font-light">{item.label}</h5>
                            ))}
                        </div>
                    ))}
                ...
                {education.map((item) => (
                    <div key={item.id} className="mt-3 mb-2 last:pb-0 pb-8 border-l border-gray-400/30">
                        <div className="flex justify-start items-start gap-4">
                            ...
                            <div className="-mt-1">
                                <h4 className="font-light text-gray-200 text-sm">{item.label}</h4>
                                <h5 key={item.id} className="mt-1 font-light text-xs text-portfolio-primary">{item.link}</h5>
                            </div>
                        </div>
                    </div>
                ))}
